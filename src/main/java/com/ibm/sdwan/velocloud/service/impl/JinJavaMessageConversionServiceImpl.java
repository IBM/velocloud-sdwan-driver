...
import com.ibm.sdwan.velocloud.utils.FileUtils;
...
public class JinJavaMessageConversionServiceImpl implements MessageConversionService {
    ...
    public String generateMessageFromRequest(String messageType, ExecutionRequest executionRequest) throws MessageConversionException {
        ...
        try {
            String returnVal = jinjava.render(template, createJinJavaContext(executionRequest.getProperties(), template, executionRequest.getDeploymentLocation()));
    ...
    private Map<String, Object> convertJsonToMap(String jsonMessage) {
        ...
        try {
            ...
            logger.error("Exception raised parsing json to map using jackson library", e);
        }
        return map;
    }
    ...
    private Map<String,Object> createJinJavaContext(Map<String, Object> resourceProperties, String templateContents, ResourceManagerDeploymentLocation deploymentLocation) throws  IOException {
        Map<String, Object> context = new HashMap<>();
        List<String> list = findPropertyListFromTemplate(templateContents);
        logger.debug("list of properties in template {}", list);
        //getting enterpriseId from deploymentLocation and remaining properties from resourceProperties.
        list.forEach(property ->{
            Object value = resourceProperties.get(property);
            logger.debug("property -->{} value ---> {}", property, value);
            if (!property.equals("enterpriseId") && value == null) {
                throw new MissingPropertyException("Missing value for resource property: " + property);
            }
            context.put(property, value);
        });
        context.put("enterpriseId", deploymentLocation.getProperties().get("enterpriseId"));
        return context;
    }
